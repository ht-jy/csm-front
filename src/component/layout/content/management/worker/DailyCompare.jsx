...
/**
 * @description: 근로자 비교(TBM, 퇴직공제)
 * 
 * @author 작성자: 김진우
 * @created 작성일: 2025-05-18
 * @modified 최종 수정일: 2025-09-01
 * @modifiedBy 최종 수정자: 정지영
 * @modified description
 * - 프로젝트 삭제 시 데이터 초기화 및 검색옵션 추가(전화번호 검색 시 실제로는 -를 제외시키고 USER_ID 검색)
 * 
 * @additionalInfo
 * 
 */
const DailyCompare = () => {
    ...
    const columns = [
        ...
        { isSearch: false, isOrder: true, width: "120px", header: "소속", itemName: "department", bodyAlign: "left", isEllipsis: true },
        { isSearch: false, isOrder: false, width: "100px", header: "전화번호", itemName: "user_id", bodyAlign: "center", isEllipsis: true, isFormat: true, format: "formatMobileNumber"},
        ...
        { isSearch: false, isOrder: false, width: "150px", header: "생체인식기", itemName: "worker_in_time", bodyAlign: "center", isEllipsis: true, colSpan: 2},
    ...
    const searchOptions = [
        ...
        { value: "USER_ID", label: "전화번호" },
        { value: "DEVICE_NM", label: "인식기명" },
    ];

    const [checkedList, setCheckedList] = useState([]);

    // 파일 div open/close -> 현재 삭제
    ...
    const fileLabelClick = (type) => {
        ...
        const fileModalText = {
            "TBM":
            <div> 
                <div>엑셀에 작성된 회사명과 날짜가 아닌 <span style={{fontWeight:"bold"}}>선택한 회사명과 근무날짜로 저장</span>이 됩니다.</div>
                <br/>
                <div>※ 기존에 올린 파일이 있다면 삭제되며, 새롭게 올린 파일에 서명된 근로자만 비교 리스트에 적용이 됩니다.</div>
                <br/>
                <div>※ 오른쪽 상단에서 다운 받은 TBM 파일과 양식이 다르다면 정상적으로 적용이 안 될 수도 있습니다.</div>
                <br/>
            </div>,            
            
            "DEDUCTION": 
                <div style={{}}>
                    <div>상단에 선택한 프로젝트가 속한 현장만 저장이 됩니다.</div>
                    <br/>
                    <div>현재 화면에서 선택한 근로날짜에 해당하는 데이터만 저장이 됩니다.</div>
                    <div>전체근로자의 <span style={{fontWeight:"bold"}}>근로자 정보(현장, 이름, 업체명, 아이디(전화번호), 생년월일)와 동일</span>하여야 합니다. </div>
                    <br/>
                    <div>※ 기존에 올린 파일이 있다면 삭제되며, 새롭게 올린 파일에 작성된 근로자만 비교 리스트에 적용이 됩니다.</div>
                    <br/>
                    <div>※ 공사관리시스템에 입력된 프로젝트명, 소속업체와 동일하게 작성되지 않으면 동일 근로자로 등록이 안 될 수도 있습니다.</div>
                    <br/>
                    <div>※ 퇴직공제 사이트에서 받은 파일이 아니거나 오른쪽 상단에서 다운 받은 퇴직공제 양식과 다르다면 적용이 안 될 수도 있습니다.</div>
                    <br/>
                </div>
        }
    ...
    // 전화번호 검색 시 백엔드에서는 USER_ID로 검색하기 때문에 -를 제외하기 위한 함수
    /*
    - FIXME: 02를 검색했는데 010-2111-1111이 검색될 수 있는 문제가 있음.
    */
    const handleUpdateRetrySearch = (searchText) => {
        const newSearchText = searchText
            .split("~")
            .map((item) => {
                const [key, value] = item.split(":");
                return key === "USER_ID" ? `${key}:${value.replace("-", "")}` : item;
            })
            .join("~");

        handleRetrySearch(newSearchText)
    }
        ...
        <div>
            ...
            <div className="container-fluid px-4">
                ...
                <div className="table-header" >
                    <div className="table-header-left" style={{gap:"10px", width:"100%"}}>
                ...
                <div className="table-header" >
                    ...
                    <div className="table-header-right">
                        ...
                        <Search 
                            ...
                            fncSearchKeywords={(searchText) => handleUpdateRetrySearch(searchText)}
                ...
                <div className="table-wrapper">
                    <div className="table-container" id="table-container" style={{overflow: "auto", maxHeight: retrySearchText ? "calc(100vh - 400px)" :"calc(100vh - 370px)"}}>
