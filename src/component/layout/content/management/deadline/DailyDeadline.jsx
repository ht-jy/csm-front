...
const DailyDeadline = () => {
    ...
    const isFile = (type) => {
        ...
        const some = state.list.some(item => item?.file_type === type)
        if(some){
            return true;
        }else{
            return false;
    ...
    const excelDownload = async(type) => {
        ...
        try {
            setIsLoading(true);
            res = await Axios.POST_BLOB(`/excel/export?file_type=${type}&work_date=${searchDate}&jno=${project.jno}`);
            console.log(res);
                                    ...
                                    excelType.map((item, idx) => (
                                        <div className="file-upload">
                                            ...
                                            <label className="file-label">
                                                ...
                                                {
                                                    isFile(item) ?
                                                        state.list.find(obj => obj.file_type === item).file_name
                                                    :
                                                        "파일을 선택하세요."
                                                }
                                                </span>
